    ...
    <div className="space-y-8 max-w-xl">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-xs text-muted-foreground">{homeTimezone}</p>
        </div>

        {windows.length === 0 && !showWindowForm && (
          <p className="text-sm text-muted-foreground mb-3">No recurring availability set — managers can still assign you.</p>
        )}

        <div className="space-y-2 mb-3">
          {windows.map((w) => (
            <div key={w.id} className="bg-card border border-border rounded-md px-3 py-2 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <Badge variant="outline" className="text-[10px] w-8 justify-center">{DAYS[w.dayOfWeek]}</Badge>
        ...
        {showWindowForm ? (
          ...
          <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setShowWindowForm(true)}>
      ...
      <div>
        ...
        {exceptions.length === 0 && !showExceptionForm && (
          <p className="text-sm text-muted-foreground mb-3">No exceptions scheduled.</p>
        )}

        <div className="space-y-2 mb-3">
          {exceptions.map((e) => (
            <div key={e.id} className="bg-card border border-border rounded-md px-3 py-2 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
        ...
        {showExceptionForm ? (
          <div className="bg-card border border-border rounded-md p-3 space-y-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button size="sm" className="bg-teal-600 hover:bg-teal-700 text-white" disabled={createMutation.isPending || !newException.date}
          ...
          <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setShowExceptionForm(true)}>
            <Plus className="h-3.5 w-3.5 mr-1" /> Add Exception
          </Button>
        )}
      </div>
    </div>
